...
void boid_update(int cur)
{
    ...
    *vy = *vy + multfix15(close_dy,avoidfactor);
    ...
    if((sheep_dog_dx < sheepDogRange) && (sheep_dog_dy < sheepDogRange) && (sheep_dog_dx > multfix15(-1, sheepDogRange)) && (sheep_dog_dy > multfix15(-1, sheepDogRange))) {
        fix15 sheep_dog_squared_distance = multfix15(sheep_dog_dx,sheep_dog_dx) + multfix15(sheep_dog_dy,sheep_dog_dy);
        if (sheep_dog_squared_distance < sheepDogRange ) {
          if(sheep_dog_dx >= 0) {
            *vx += sheep_shift_factor;
          } else {
            *vx -= sheep_shift_factor;
          }
          if(sheep_dog_dy >= 0) {
            *vy += sheep_shift_factor;
          } else {
            *vy -= sheep_shift_factor;
          }
        }
