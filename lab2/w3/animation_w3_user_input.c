...
fix15 bias_increment = float2fix15(0.0004);
...
void boid_update(int cur)
{
    ...
    *vy = *vy + multfix15(close_dy,avoidfactor);
    ...
    // if(default_bias_change == 1) {
    //   *bias = biasval;
    //   default_bias_change = 0;
    // }
    ...
    else if (cur >= boid_set1_num && cur <  boid_set1_num + boid_set2_num) {
      if (boid_dynamic_bias_enable == 1) {
        if (*vx < 0){
          *bias = min(maxbias, *bias + bias_increment);
        }
...
static PT_THREAD (protothread_serial(struct pt *pt))
{
    ...
    serial_write ;
      while(1) {
        ...
        // print prompt
        sprintf(pt_serial_out_buffer, "input set1 boids: (0 for no change)");
        // non-blocking write
        serial_write ;
        // spawn a thread to do the non-blocking serial read
        serial_read ;
        // convert input string to number
        sscanf(pt_serial_in_buffer,"%d", &user_input) ;
        if (user_input > 0 && user_input < 500) {
         boid_set1_num =  user_input;
        }

        // print prompt
        sprintf(pt_serial_out_buffer, "input set2 boids: (0 for no change)");
        // non-blocking write
        serial_write ;
        // spawn a thread to do the non-blocking serial read
        serial_read ;
        ...
        sscanf(pt_serial_in_buffer,"%d", &user_input) ;
        if (user_input > 0 && user_input < 500) {
         boid_set2_num =  user_input;
        }

        // print prompt
        sprintf(pt_serial_out_buffer, "input bias strength: (0 for no change)");
        // non-blocking write
        serial_write ;
        // spawn a thread to do the non-blocking serial read
        serial_read ;
        ...
        sscanf(pt_serial_in_buffer,"%d", &user_input) ;
        if (user_input > 0) {
          biasval = int2fix15(user_input);
          default_bias_change = 1;
        }

        // print prompt
        sprintf(pt_serial_out_buffer, "Enable Dynamic Bias? (1 for yes, 0 for no)");
        // non-blocking write
        serial_write ;
        // spawn a thread to do the non-blocking serial read
        serial_read ;
...
static PT_THREAD (protothread_anim(struct pt *pt))
{
    ...
    for(int i = 0; i < boid_total_num; i++) {
        ...
        else if (i >= boid_set1_num && i <  boid_set1_num + boid_set2_num) {
