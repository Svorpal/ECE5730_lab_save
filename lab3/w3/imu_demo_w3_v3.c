...
volatile int motor_disp_P = 0;
volatile int motor_disp_I = 0;
volatile int motor_disp_D = 0;
...
void on_pwm_wrap() {
    ...
    // Update duty cycle
    int Kp = fix2int15((multfix15(int2fix15(Para_P) , error)));
    int Ki = fix2int15(multfix15(int2fix15(Para_I) , error_accumulation));
    int Kd = fix2int15(multfix15(int2fix15(Para_D) , error_deriv));
    control = Kp + Ki + Kd ;
    prev_error = error ;
    ...
    motor_disp_P = motor_disp_P + ((Kp - motor_disp_P)>>6) ;
    motor_disp_I = motor_disp_I + ((Ki - motor_disp_I)>>6) ;
    motor_disp_D = motor_disp_D + ((Kd - motor_disp_D)>>6) ;
    ...
    while (true) {
        ...
        if (throttle >= threshold) { 
           ...
           drawVLine(xcoord, 0, 480, BLACK) ;
            ...
            // Draw top plot
            drawPixel(xcoord, 230 - (225 - 75) / 2 - motor_disp_P / 63, RED) ;
            drawPixel(xcoord, 230 - (225 - 75) / 2 - motor_disp_I / 63, GREEN) ;
            drawPixel(xcoord, 230 - (225 - 75) / 2 - motor_disp_D / 63, BLUE) ;
